...
from repository import idempotency_repo
...
def test_repeated_request_id_replays_instead_of_writing_twice(db) -> None:
    """Double-click protection: the same request_id cannot write twice.

    Resolved 2026-09-22 in favour of CLAUDE.md rule 12 — the repeat REPLAYS the
    original result rather than erroring, so a double-clicked save shows the
    same success twice instead of a red error after a save that worked. What
    must never change is that nothing is written the second time.
    """
    user = _user(db)
    first = _save(db, user, request_id="REQ-SAME")

    second = _save(db, user, request_id="REQ-SAME")

    assert second.audit_id == first.audit_id, "the ORIGINAL audit id, not a new one"
    assert second.allocation_date == first.allocation_date
    assert second.allocation_records == first.allocation_records
    assert second.flow_records == first.flow_records
    assert second.totals == first.totals, "totals must survive the round trip"

    # The decisive assertion: one save, one set of rows, one audit entry.
    assert db.scalar(select(func.count()).select_from(MetalMaster)) == first.allocation_records
    assert (
        db.scalar(
            select(func.count())
            .select_from(MetalAllocationAuditLog)
            .where(MetalAllocationAuditLog.action_type == "SAVE")
        )
        == 1
    )


def test_a_repeated_request_id_from_another_user_is_refused_not_replayed(db) -> None:
    """request_id is generated by the browser, so it is guessable and is not a
    secret. Replaying across accounts would hand a stranger somebody else's
    audit id, record counts and totals — so the repeat is refused instead."""
    user = _user(db)
    _save(db, user, request_id="REQ-SHARED")

    other = AppUser(
        email="someone.else@royalchains.com",
        display_name="Other",
        is_admin=True,
        admin_denied=False,
        is_active=True,
    ...
    with pytest.raises(DuplicateRequestError) as exc:
        _save(db, other, request_id="REQ-SHARED")
    assert exc.value.code == "DUPLICATE_REQUEST"


def test_a_repeated_request_id_from_another_endpoint_is_refused(db) -> None:
    """The three write paths store three different shapes; replaying one into
    another would return a result of the wrong kind."""
    from services.idempotency_service import replayable_result

    user = _user(db)
    _save(db, user, request_id="REQ-ENDPOINT")

    entry = idempotency_repo.find_live_entry(db, "REQ-ENDPOINT")
    assert entry is not None
    assert (
        replayable_result(entry, user_email=user.email, endpoint="submit_requirements")
        is None
    )
    assert (
        replayable_result(entry, user_email=user.email, endpoint="POST /allocations")
        is not None
    )


def test_an_unreadable_stored_result_falls_back_to_the_duplicate_error(db) -> None:
    """Rows written by an earlier build carry no response_json. Degrading to the
    old error is always safe — it never double-writes."""
    user = _user(db)
    _save(db, user, request_id="REQ-OLD")

    entry = idempotency_repo.find_live_entry(db, "REQ-OLD")
    entry.response_json = "{not json"
    db.commit()

    with pytest.raises(DuplicateRequestError) as exc:
        _save(db, user, request_id="REQ-OLD")
    assert exc.value.code == "DUPLICATE_REQUEST"
    # Still exactly one save: the refusal did not become a second write.
    assert db.scalar(select(func.count()).select_from(MetalMaster)) == 1
